    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
            ...
            @if(Auth::user()->role === 'admin')
                <a href="{{ route('admin.transactions.create') }}" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition duration-200">
                    Add Transaction
                </a>
            @else
                    ...
                    <tbody>
                            ...
                            <td class="py-3 px-4">
                                <div class="flex space-x-2">
                                    @if(Auth::user()->role === 'admin')
                                        ...
                                        <a href="{{ route('admin.transactions.edit', $transaction->id) }}" class="text-amber-400 hover:text-amber-300 ml-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                            </svg>
                                        </a>
                                        @if($transaction->receipt_id)
                                        <a href="{{ route('admin.receipts.print', $transaction->receipt_id) }}" target="_blank" class="text-green-400 hover:text-green-300 ml-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        ...
                                        @else
                                        <form method="POST" action="{{ route('admin.transactions.receipt', $transaction->id) }}" class="inline" onsubmit="return confirm('Generate receipt for this transaction?')">
                                            @csrf
                                        ...
                                        @endif
                                        <form action="{{ route('admin.transactions.destroy', $transaction->id) }}" method="POST" class="inline ml-2">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="py-8 px-4 text-center text-gray-500">
                                ...
                                @if(Auth::user()->role === 'admin')
                                    <a href="{{ route('admin.transactions.create') }}" class="text-amber-400 hover:text-amber-300">Add your first transaction</a>.
                                @else
                                    <a href="{{ route('bendahara.transactions.create') }}" class="text-amber-400 hover:text-amber-300">Add your first transaction</a>.
                                @endif
                            </td>
                        </tr>
                        @endforelse
